...
public class EmployeeModel {
    ...
    public List<Employee> findAll() throws SQLException {
        ...
        while (rs.next()) {
            ...
            products.add(new Employee(id, name, phone, status));
        }
        return products;
    }
    ...
    public Employee findById(int id) throws SQLException {
        ...
        if (rs.next()) {
            ...
            String phone = rs.getString("phone");
            int status = rs.getInt("status");
            return new Employee(id, name, phone, status);
        }
        return null;
    }

    public Employee save(Employee product) throws SQLException {
        PreparedStatement stmt = conn.prepareStatement("insert into employees (name, phone, status) values (?, ?, ?)", Statement.RETURN_GENERATED_KEYS);
        stmt.setString(1, product.getName());
        stmt.setString(2, product.getPhone());
    ...
    public Employee update(int id, Employee updateProduct) throws SQLException {
        PreparedStatement stmt = conn.prepareStatement("update employees set name = ?, phone = ?, status = ? where id = ?");
        stmt.setString(1, updateProduct.getName());
        stmt.setString(2, updateProduct.getPhone());
